...
const Tarea = ({tarea, proyectoId}) => {
    ...
    const cambiarEstado = async () => {
        ...
        try {
            const { data } = await actualizarTarea({                // La actualización, actualiza el caché automáticamente, no como cuando insertabamos un nuevo registro.
                variables:{
                    id,
                    input:{
                        nombre: tarea.nombre
    ...
    // Diálogo para eliminar o no una tarea
    const mostrarEliminar = () => {
        Alert.alert('Eliminar Tarea', '¿Deseas eliminar esta tarea?',[
            ...
            {
                text: 'Confirmar',
                onPress: () => eliminarTareaDB()       // No la hemos llamado eliminarTarea porque la función que traeremos desde el resolver de GraphQL ya se llama eliminarTarea.
            }
        ])
    }
